...
class SnippetManager(QMainWindow):
    def __init__(self):
        ...
        self.main_folder = self.default_folder
        ...
        folder = settings.value("main_folder", self.default_folder)
        if folder and os.path.exists(folder):
            self.main_folder = folder
        self.populate_tree(self.main_folder)
    ...
    def populate_tree(self, main_folder):
        print(f"Populating tree with folder: {main_folder}")
        self.tree_model.clear()
        root_item = self.tree_model.invisibleRootItem()
        projects = os.listdir(main_folder)
    ...
    def on_tree_selection_changed(self):
        ...
        if selected:
            ...
            if item.hasChildren():  # 项目
                project_path = os.path.join(self.main_folder, item.text())
    ...
    def open_folder(self):
        ...
        if folder:
            ...
            settings = QSettings("MyCompany", "SnippetManager")
            settings.setValue("main_folder", folder)
